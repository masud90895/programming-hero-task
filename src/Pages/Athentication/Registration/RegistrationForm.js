...
const RegistrationForm = () => {
  ...
  const RegistrationData = (data) => {
    ...
    fetch(`${process.env.REACT_APP_serverURL}/api/registration`, {
      ...
      .then((result) => {
        console.log(result);
        if (result.status === "ok") {
          fetch(`${process.env.REACT_APP_serverURL}/jwt`, {
            method: "POST",
            headers: {
              "content-type": "application/json",
            },
  ...
  return (
    <div className="flex flex-col max-w-md mx-auto p-6 rounded-md sm:p-10 bg-[#bfe6f8] mt-10">
      ...
      <form
        onSubmit={handleSubmit(RegistrationData)}
      ...
      >
        <div className="space-y-4">
          ...
          {errors.email && (
            <span className="text-red-600">Email Fild is required</span>
          )}
          <div>
            <div className="flex justify-between mb-2">
              <label className={`text-sm ${errors.password && "text-red-600"}`}>
                Password
              </label>
            </div>
            <input
        ...
        <div className="space-y-2">
          <div>
            <button
              type="submit"
              className="w-full px-8 py-3 font-semibold rounded-md bg-[#219ebc] text-gray-900 hover:bg-[#bfe6f8] border border-[#bfe6f8] hover:border-[#219ebc]"
            >
              Register
            </button>
          </div>
          <p className="px-6 text-sm text-center text-gray-700">
            ...
            <Link to="../login" className="hover:underline text-[#219ebc]">
              Login
            </Link>
            .
          </p>
